...
__global__ void update_tiling_one_to_one(float* Ex, float* Ey, float* Ez,
  ...
  int local_x = thread_id % NTX;
  int local_y = (thread_id / NTX) % NTY;
  int local_z = thread_id / (NTX * NTY);
...
__global__ void update_tiling_one_to_two(float* Ex, float* Ey, float* Ez,
                                         float* Hx, float* Hy, float* Hz,
                                         float* Cax, float* Cay, float* Caz,
                                         float* Cbx, float* Cby, float* Cbz,
                                         float* Jx, float* Jy, float* Jz,
  ...
  // Compute local thread coordinates within the tile
  int new_NTX = NTX / 2;
  int local_x = thread_id % new_NTX;
  int local_y = (thread_id / new_NTX) % NTY;
  int local_z = thread_id / (new_NTX * NTY);
  ...
  // second element
  Ex[global_idx + new_NTX] = Cax[global_idx + new_NTX]*Ex[global_idx + new_NTX] + Cbx[global_idx + new_NTX] * (Hz[global_idx + new_NTX] + Hy[global_idx + new_NTX] + Jx[global_idx + new_NTX]) * dx; 
  Ey[global_idx + new_NTX] = Cay[global_idx + new_NTX]*Ey[global_idx + new_NTX] + Cby[global_idx + new_NTX] * (Hx[global_idx + new_NTX] + Hz[global_idx + new_NTX] + Jy[global_idx + new_NTX]) * dx; 
  Ez[global_idx + new_NTX] = Caz[global_idx + new_NTX]*Ey[global_idx + new_NTX] + Cbz[global_idx + new_NTX] * (Hx[global_idx + new_NTX] + Hy[global_idx + new_NTX] + Jz[global_idx + new_NTX]) * dx; 
...
void gpu_tiling_one_to_two(std::vector<float>& Ex, std::vector<float>& Ey, std::vector<float>& Ez, 
                           std::vector<float> Hx, std::vector<float> Hy, std::vector<float> Hz,
                           std::vector<float> Cax, std::vector<float> Cay, std::vector<float> Caz,
                           std::vector<float> Cbx, std::vector<float> Cby, std::vector<float> Cbz,
                           std::vector<float> Jx, std::vector<float> Jy, std::vector<float> Jz,
  ...
  // now each thread in X dimension will handle 2 elements!
  size_t block_size = (NTX / 2) * NTY * NTZ;
  size_t grid_size = xx_num * yy_num * zz_num;
  auto start = std::chrono::steady_clock::now();
  for(int t=0; t<timesteps; t++) {
    update_tiling_one_to_two<<<grid_size, block_size>>>(Ex_d, Ey_d, Ez_d,
                                                        Hx_d, Hy_d, Hz_d,
                                                        Cax_d, Cay_d, Caz_d,
                                                        Cbx_d, Cby_d, Cbz_d,
                                                        Jx_d, Jy_d, Jz_d,
  ...
  auto end = std::chrono::steady_clock::now();
  size_t gpu_runtime = std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();
  std::cout << "gpu tiling one_to_two runtime(excluding memcpy): " << gpu_runtime << "us\n";
...
int main(int argc, char* argv[]) {
  ...
  std::vector<float> Ex_gpu_tiling_one_to_two(array_size, 1.0);
  std::vector<float> Ey_gpu_tiling_one_to_two(array_size, 1.0);
  std::vector<float> Ez_gpu_tiling_one_to_two(array_size, 1.0);
  ...
  gpu_tiling_one_to_two(Ex_gpu_tiling_one_to_two, Ey_gpu_tiling_one_to_two, Ez_gpu_tiling_one_to_two, 
                        Hx, Hy, Hz,
                        Cax, Cay, Caz,
                        Cbx, Cby, Cbz,
                        Jx, Jy, Jz,
  ...
  for(size_t i=0; i<Nx*Ny*Nz; i++) {
    if(fabs(Ex_cpu_single[i] - Ex_gpu_tiling_one_to_two[i]) > 1e-5 ||
       fabs(Ey_cpu_single[i] - Ey_gpu_tiling_one_to_two[i]) > 1e-5 ||
       fabs(Ez_cpu_single[i] - Ez_gpu_tiling_one_to_two[i]) > 1e-5) {
      correct = false;
      break;
    }
  }
